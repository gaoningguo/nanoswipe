...
export const usePlayerStore = defineStore('player', () => {
  ...
  async function forceProxyPlayback(video) {
    if (!video || video.source !== 'moontv' || !video.url) return null
    if (video.url.includes('/api/proxy/vod/m3u8') || video.url.includes('/api/proxy-m3u8')) return video.url

    // 优先走 MoonTVPlus 的 directplay 通道：`/api/proxy-m3u8` 在 middleware 白名单里，
    // 服务端 source=directplay 跳过 proxyMode 校验，对所有源都能正常代理回内容。
    const proxyUrl = moontvApi.buildDirectProxyPlayUrl(
      sourceStore.moontvUrl,
      video.url
    )
    ...
    const idx = queue.value.findIndex(v => v.id === video.id)
    if (idx !== -1) {
      queue.value[idx].url = proxyUrl
      queue.value[idx].proxyMode = true
      queue.value[idx].proxyFallbackTried = true
    }

    video.url = proxyUrl
    video.proxyMode = true
    video.proxyFallbackTried = true
    return proxyUrl
  }
